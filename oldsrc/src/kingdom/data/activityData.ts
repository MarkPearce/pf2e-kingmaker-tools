...
<p>Rulers are particularly difficult to assign; when you take this activity to assign a new Ruler, you take a –4 circumstance penalty to the skill check, and unless you achieve a critical success, you ${gainUnrest(
      1
    )}. Whether or not you are simultaneously assigning a leader, you may also use this activity to attempt to reselect the four leadership roles that you have invested. Any result other than a critical failure allows this.</p>`,
    ...
    success: {
      msg: `The people accept the new leader. The leader immediately provides the benefits tied to occupying the new role. ${gainUnrest(
        1
      )}`,
    },
    failure: {
      msg: `The people are unsure about the new leader. The leader takes a –1 circumstance penalty to all checks they attempt as part of their activities during the Activity phase of each Kingdom turn. At the end of the next Kingdom turn, the leader can attempt any Loyalty-based basic skill check to ingratiate themselves with the populace. The leader may attempt this check at the end of each Kingdom turn until they succeed. Success removes this penalty, but a critical failure results in the development detailed in Critical Failure below. ${gainUnrest(
        1
      )}`,
      modifiers: () => [
        {
    ...
    criticalFailure: {
      msg: `The people reject the new leader. The leadership role is treated as vacant and you must attempt to reassign it using the New Leadership activity at the start of the next Kingdom turn. ${gainUnrest(
        1
      )}`,
    },
  },
  ...
  "collect-taxes": {
    ...
    success: {
      msg: `Your tax collectors gather enough to grant you a +1 circumstance bonus to Economy-based checks for the remainder of the Kingdom turn. If you attempted to Collect Taxes during the previous turn, ${gainUnrest(
        1
      )}`,
      modifiers: () => [
        {
    ...
    failure: {
      msg: `Your tax collectors gather enough to grant you a +1 circumstance bonus to Economy-based checks for the remainder of the Kingdom turn. In addition, people are unhappy about taxes—${gainUnrest(
        1
      )} (or ${gainUnrest(
        2
      )} if you attempted to Collect Taxes the previous turn).`,
      modifiers: () => [
        {
          turns: 1,
          enabled: true,
    ...
    criticalFailure: {
      msg: `Your tax collectors encounter resistance from the citizens and their attempts to gather taxes are rebuffed. While the tax collectors still manage to gather enough taxes to support essential government, they have angered the kingdom's citizens and encouraged rebellious acts. ${gainUnrest(
        2
  ...
  "improve-lifestyle": {
    ...
    criticalFailure: {
      // TODO: ruin of choice
      msg: `Your attempt to Improve Lifestyles backfires horribly as criminal elements in your kingdom abuse your generosity. You take a –1 circumstance penalty to Economy-based checks for the remainder of the Kingdom turn, ${gainUnrest(
        1
      )}, and add 1 to a Ruin of your choice.`,
      modifiers: () => [
        {
          turns: 1,
          enabled: true,
  ...
  "tap-treasury": {
    ...
    criticalFailure: {
      msg: `You fail to secure the funds you need, and rumors about the kingdom’s potential shortfall of cash cause you to take a –1 circumstance penalty to all Loyalty- and Economy-based checks until the end of your next Kingdom turn. In addition, rumors spiral out of control. ${gainUnrest(
        1
      )} and add 1 to a Ruin of your choice.`,
      modifiers: () => [
        {
  ...
  "trade-commodities": {
    ...
    criticalFailure: {
      msg: `You gain no bonus Resource Dice (though the Commodity remains depleted). If you Traded Commodities the previous turn, ${gainUnrest(
        1
      )}`,
    },
  },
  "manage-trade-agreements": {
    ...
    description: `You send agents out to attend to established trade agreements. ${loseRP(
      2,
      true
    )} per Trade Agreement you wish to manage. Then attempt a basic check. If you Managed Trade Agreements on the previous turn, increase this DC by 5.`,
  ...
  "mark-test": {
    ...
    requirement:
      "You must be within the influence of a settlement that contains at least one Bank.",
    skills: simpleRank(["trade"]),
    criticalSuccess: {
      msg: `Success Your kingdom reaps the benefits of your investment. ${gainRolledRD(
        4
      )}`,
    },
    success: {
    ...
    failure: {
      msg: `Your investment ends up being used to shore up shortfalls elsewhere. ${gainRP(
        "1d4"
      )}`,
    },
    criticalFailure: {
      // TODO: increase crime by an equal amount
      msg: `Your investment is embezzled, lost, or otherwise misappropriated. Choose one of the following: either ${gainRolledRD(
        1
      )} and also increase your Crime by an equal amount, or gain 0 RP and ${gainRuin(
        "crime",
        1
      )}`,
    },
...
You can use Capital Investment to repay funds from Tap Treasury (page 528). In this case, no roll is needed and you simply deduct the appropriate amount of funds from your personal wealth to pay back that which was borrowed. When you use Capital Investment to generate RP, the amount of gp required to make an investment is set by your kingdom’s level. Investments below this amount cause your attempt at Capital Investment to suffer an automatic critical failure, while investments above this amount are lost. The investment required is equal to the value listed on Table 10–9: Party Treasure by Level @UUID[Compendium.pf2e.journals.S55aqwWIzpQRFhcq.JournalEntryPage.Ly8l2GT6dbvuY3A2]{Treasure}; use the value for your kingdom’s level under the “Currency per Additional PC” as the required investment value. This is a basic check.`,
  ...
  "celebrate-holiday": {
    ...
    success: {
      msg: `Your holidays are a success, but they’re also expensive. You gain a +1 circumstance bonus to Loyalty-based checks until the end of your next Kingdom turn. ${loseRolledRD(
        1
      )}. If you can’t afford this cost, treat this result as a Critical Failure instead.`,
      modifiers: () => [
        {
          turns: 2,
          enabled: true,
    ...
    failure: {
      msg: `The holiday passes with little enthusiasm, but is still expensive. ${loseRolledRD(
        1
      )}. If you can’t afford this cost, treat this result as a Critical Failure instead.`,
    },
    criticalFailure: {
  ...
  "clandestine-business": {
    ...
    criticalSuccess: {
      msg: `${gainRolledRD(2)}. In addition, you ${gainCommodities(
        "luxuries",
        "1d4"
      )}. The public is none the wiser.`,
    },
    success: {
      msg: `Either ${gainRolledRD(2)}, or gain ${gainCommodities(
        "luxuries",
        "1d4"
      )}. Regardless of your choice, rumors spread about where the government is getting these “gifts.” ${gainUnrest(
        1
      )}.`,
    },
    failure: {
      msg: `${gainRolledRD(
        1
      )}. Rumors are backed up with eyewitness accounts. ${gainUnrest(
        1
      )} and ${gainRuin("corruption", 1)}.`,
    },
    criticalFailure: {
      msg: `You gain nothing from the Clandestine Business but angry citizens. ${gainUnrest(
        "1d6"
      )}, ${gainRuin(
        "corruption",
        2
      )}, and one other Ruin of your choice by 1.`,
    },
  },
  "craft-luxuries": {
    oncePerRound: false,
    ...
    description: `You encourage your artisans to craft luxury goods and may even aid them in this pursuit. ${loseRolledRD(
      1
    )}. Then attempt a basic check.`,
    skills: simpleRank(["arts"]),
    criticalSuccess: {
      msg: `Your artisans exceed expectations and craft extravagant goods. ${gainCommodities(
        "luxuries",
        "1d4"
      )}`,
    },
    success: {
      msg: `Your artisans produce some delightful goods. ${gainCommodities(
        "luxuries",
        1
      )}`,
    },
  ...
  "create-a-masterpiece": {
    ...
    criticalFailure: {
      msg: `Not only does your attempt to create a masterpiece fail, it does so in a dramatic and humiliating way. ${loseFame(
        1
      )}; if you have no Fame or Infamy points to lose, instead ${gainUnrest(
        "1d4"
      )}`,
    },
  },
  "creative-solution": {
    ...
    criticalSuccess: {
      msg: `You can call upon the solution to aid in resolving any Kingdom skill check made during the remainder of this Kingdom turn ${gainSolution(
        "creative-solution"
      )}. Do so when a Kingdom skill check is rolled, but before you learn the result. Immediately reroll that check with a +2 circumstance bonus; you must take the new result. If you don’t use your Creative Solution by the end of this turn, you lose this benefit and gain 10XP instead.`,
      modifiers: () => [
        {
          turns: 1,
          enabled: false,
    ...
    success: {
      msg: `You can call upon the solution to aid in resolving any Kingdom skill check made during the remainder of this Kingdom turn ${gainSolution(
        "creative-solution"
      )}. Do so when a Kingdom skill check is rolled, but before you learn the result. Immediately reroll that check with a +2 circumstance bonus; you must take the new result. If you don’t use your Creative Solution by the end of this turn, you lose this benefit and gain 10XP instead. In addition, ${loseRP(
        "1d4"
      )} to research the solution. This cost is paid now, whether or not you use your Creative Solution.`,
      modifiers: () => [
        {
          turns: 1,
          enabled: false,
    ...
    failure: {
      msg: `Your attempt at researching is a failure and you ${loseRP(
        "2d6"
    ...
    criticalFailure: {
      msg: `Your attempt at researching is a failure and you ${loseRP(
        "2d6"
      )}. It provides no advantage. In addition, your scholars and thinkers are so frustrated that you take a –1 circumstance penalty to Culture-based checks until the end of the next Kingdom turn.`,
      modifiers: () => [
        {
          turns: 2,
          enabled: true,
    ...
    special:
      "You cannot influence a check with Supernatural Solution and Creative Solution simultaneously.",
  },
  "decadent-feasts": {
    oncePerRound: false,
    fortune: false,
    ...
    description: `Your goddess is more than just the goddess of undeath— she’s also the goddess of gluttony. And you are no fool; you understand the fear your goddess inspires in the living and knows that focusing on other aspects of her worship are more likely to result in positive growth for the nation. In order to distract the populace, you arrange for decadent feasts for the people, simultaneously feeding the hungry while camouflaging some of your faith’s more sinister aspects. ${loseCommodities(
      "food",
      "1d8"
    )} and ${loseCommodities(
      "luxuries",
      1
    )}, then attempt a basic Agriculture or Trade check to determine how effective the feasts are.`,
    skills: simpleRank(["agriculture"]),
    criticalSuccess: {
      msg: `The people rejoice and glut themselves on the repast! ${loseUnrest(
        "1d6"
      )}, and the next time this Kingdom turn you suffer an effect that increases Unrest, do not increase your Unrest.`,
    },
    success: {
      msg: `The people enjoy the meal, but no longer than it takes to gulp it down. ${loseUnrest(
        "1d3"
      )}`,
    },
    failure: {
      msg: `The meal is appreciated, but the people remain suspicious. ${loseUnrest(
        1
      )}.`,
    },
    criticalFailure: {
      msg: `The people are too suspicious of the feast to enjoy it. Worse, rumors that Jaethal is trying to distract the citizens from an evil ritual spread. ${gainUnrest(
        "1d4"
      )} and ${gainRuin("corruption", 1)}`,
    },
  },
  "deliberate-planning": {
    ...
    description:
      "Yout take time and weigh all options when faced with decisions, regardless of their importance. While this can sometimes lead you to taking too long to make choices, your theoretical analysis can be quite helpful in navigating continuous events. Choose a single continuous event that will affect your kingdom on this turn’s Event Phase, then attempt a Scholarship check against that event’s DC.",
...
The check’s DC is either the group’s Negotiation DC (see sidebar) or your kingdom’s Control DC, whichever is higher.`,
    ...
    criticalSuccess: {
      msg: `You successfully establish a trade agreement with your target, and your merchants return with gifts! ${gainRolledRD(
        2
      )}`,
    },
    success: {
    ...
    failure: {
      msg: `Your traders reach their destination but need to sweeten the deal to secure the trade agreement. ${loseRolledRD(
        2
      )}. If you do so, you successfully establish a trade agreement, otherwise the attempt fails.`,
    },
    criticalFailure: {
      msg: `Your trade agreement is a total loss and your traders do not return. ${gainUnrest(
        1
      )}, and until the end of the next Kingdom turn, take a –1 circumstance penalty to all Economy-related checks.`,
      modifiers: () => [
        {
          turns: 2,
          enabled: false,
  ...
  "evangelize-the-dead": {
    ...
    skills: simpleRank(["folklore"]),
    criticalSuccess: {
      msg: `Your prayers soothe and calm the more criminal-minded citizens for the time being. ${loseUnrest(
        "1d4"
      )}, and either ${loseRuin("crime", 2)} or ${loseRuin(
        "corruption",
        1
      )} or ${loseRuin("strife", 1)}.`,
    },
    success: {
      msg: `Your prayers serve to redirect and calm discord. ${loseUnrest(
        "1d3"
      )}`,
    },
    failure: {
    ...
    criticalFailure: {
      msg: `Your prayers have unsettled some of your citizens. ${gainUnrest(
        "1d4"
      )} and ${gainRuin("decay", 1)} `,
    },
  },
  "false-victory": {
    oncePerRound: false,
    ...
    title: "False Victory",
    description:
      "Your contacts with the criminal underworld and your knack for dodging punishment and claiming responsibility for victories you had no direct role in can be harnessed to engineer false victories to trick the kingdom’s citizens into thinking their leaders are doing more than they actually are to create a safe place to live. Such attempts are not without risks, though, for if things backfire, you can cause problems where none existed in the first place. When setting up a false victory, attempt a basic Intrigue check.",
    skills: simpleRank(["intrigue"]),
    criticalSuccess: {
      // TODO: ruin of your choice
      msg: `At the end of this Kingdom turn’s Event Phase, roll again on the random kingdom events table. Rumors of this event being resolved spread throughout your kingdom. You don’t gain any of the benefits of resolving this false victory, but instead ${loseUnrest(
        "1d6"
      )} and one Ruin of your choice by 1. If you randomly roll that same random kingdom event at any time during the next four kingdom turns, you can attempt an Intrigue check with a +1 circumstance bonus to resolve it rather than the normal check to resolve it.`,
      modifiers: () => [
        {
          turns: 5,
          enabled: false,
    ...
    success: {
      msg: `Vague rumors of the kingdom’s leaders attaining victories over vague threats spread through the kingdom. ${loseUnrest(
        "1d3"
      )}`,
    },
    failure: {
      msg: `The false event fails to manifest, and rumors of the truth spread throughout the kingdom. ${gainUnrest(
        1
      )}. You cannot attempt False Victory on your next Kingdom turn.`,
    },
    criticalFailure: {
...
The Cooperative Leadership Kingdom feat (page 531) increases the efficiency of this activity.`,
  ...
  "hire-adventurers": {
    ...
    description: `While the PCs can strike out themselves to deal with ongoing events, it’s often more efficient to Hire Adventurers. When you Hire Adventurers to help end an ongoing event, the DC is equal to your Control DC adjusted by the event’s level modifier. ${loseRolledRD(
      1
    )} each time you attempt this activity.`,
...
You can attempt this skill check with Intrigue, Statecraft, or Warfare; however, certain groups will respond better (or worse) to specific skills. The DC is the group’s Negotiation DC (see the sidebar on page 519).`,
    ...
    success: {
      msg: `The group becomes part of your kingdom, granting the specific boon or advantage listed in that group’s entry. If the hex doesn’t share a border with your kingdom, it becomes a secondary territory and checks involving this location take a Control penalty. ${loseRolledRD(
        1
      )} to the result to integrate the group into your kingdom.`,
    },
    failure: {
      msg: `The group refuses to pledge to you at this time. You can attempt to get them to Pledge Fealty next turn. ${gainUnrest(
        1
      )}`,
    },
    criticalFailure: {
      msg: `The group refuses to pledge to you—furthermore, it will never Pledge Fealty to your kingdom, barring significant in-play changes or actions by the PCs (subject to the GM’s approval). The group’s potentially violent rebuff of your offer ${gainUnrest(
        2
      )} and increases a Ruin of your choice by 1.`,
    },
  },
  "preventative-measures": {
    ...
    description:
      "You help to organize magical defenses and resources to combat potential upcoming disasters or dangers to the kingdom. Attempt a basic Magic check to determine how effective the magical preparations are.",
  ...
  "process-hidden-fees": {
    ...
    description:
      "With your aid, you can process additional taxes, fees, and payments. Attempt a basic Trade check to determine what sorts of additional resources you gather.",
    skills: simpleRank(["trade"]),
    criticalSuccess: {
      msg: createResourceButton({
        turn: "next",
    ...
    failure: {
      msg: `${createResourceButton({
        ...
        value: "1",
        type: "resource-dice",
      })}, but the citizens catch wind of the fees and grow unhappy. ${gainUnrest(
        1
      )}, and you cannot Process Hidden Fees on your next Kingdom turn.`,
    },
    criticalFailure: {
      msg: `${createResourceButton({
        turn: "next",
  ...
  "provide-care": {
    ...
    criticalSuccess: {
      msg: `You provide unexpectedly compassionate support for the people. ${loseUnrest(
        1
      )} and reduce one Ruin of your choice by 1.`,
    },
    success: {
      msg: `Your care soothes the worries and fears of the populace; ${loseUnrest(
        1
      )}.`,
    },
    failure: {
    ...
    criticalFailure: {
      msg: `Your attempt to provide care backfires. ${gainUnrest(
        1
      )} or a Ruin of your choice by 1.`,
    },
  },
  "purchase-commodities": {
    ...
    description: `You can spend RP to Purchase Commodities, but doing so is more expensive than gathering them or relying upon trade agreements. When you Purchase Commodities, select the Commodity you wish to purchase (Food, Lumber, Luxuries, Ore, or Stone). ${loseRP(
      8
    )} if you’re purchasing Luxuries or ${loseRP(
      4
    )} if you’re purchasing any other Commodity. Then attempt a basic check.`,
    skills: simpleRank(["trade"]),
    criticalSuccess: {
...
<li>${gainCommodities("luxuries", "1")}</li>
  ...
  "quell-unrest": {
    ...
    criticalFailure: {
      msg: `You not only fail to reduce Unrest, but actually incite further anger among the citizenry. Choose one of the following: ${gainUnrest(
        "1d4"
      )} or increase two Ruins of your choice by 1.`,
    },
  },
  "read-all-about-it": {
    oncePerRound: false,
  ...
  "reconnoiter-hex": {
    ...
    description: `You send a team to spend time surveying and exploring a specific hex, getting the lay of the land and looking for unusual features and specific sites. ${loseRP(
      1
    )} and then attempt a Basic check.`,
  ...
  "recruit-army": {
    ...
    criticalFailure: {
      msg: `Many of the individuals in the army you attempted to recruit took offense at the attempt. ${gainUnrest(
        1
      )}, and you cannot attempt to recruit an army again until the next Kingdom turn.`,
    },
  },
  "relocate-capital": {
    oncePerRound: false,
    ...
    success: {
      msg: `The move goes smoothly and with minimal disruption, but some folks are upset or homesick. ${gainUnrest(
        1
      )}`,
    },
    failure: {
      msg: `The move causes unhappiness. ${gainUnrest(
        1
      )} and increase two Ruins of your choice by 1.`,
    },
    criticalFailure: {
      msg: `The people reject the idea of the new capital and demand you move it back. The move is unsuccessful, and your capital remains unchanged. ${gainUnrest(
        "1d4"
      )}. Increase three Ruins of your choice by 1 and the fourth Ruin by 3.`,
    },
  },
...
The skill used to Repair Reputation depends on which Ruin total you wish to reduce. If you wish to reduce your Corruption, you attempt an Arts check. If you wish to reduce your Crime, you attempt a Trade check. If you wish to reduce your Decay, you attempt an Engineering check. If you wish to reduce your Strife, you attempt an Intrigue check. In all cases, the DC is your Control DC + 2.`,
    ...
    criticalSuccess: {
      msg: `${loseRuin(
        "corruption",
        2
      )} and reduce its current ruin penalty by 1 to a minimum of 0.`,
    },
    success: {
    ...
    criticalFailure: {
      msg: `You fail to reduce the targeted Ruin in a particularly public and embarrassing way. ${gainUnrest(
        "1d4"
      )}, and you cannot attempt to Repair Reputation for 3 Kingdom turns.`,
    },
  },
...
The skill used to Repair Reputation depends on which Ruin total you wish to reduce. If you wish to reduce your Corruption, you attempt an Arts check. If you wish to reduce your Crime, you attempt a Trade check. If you wish to reduce your Decay, you attempt an Engineering check. If you wish to reduce your Strife, you attempt an Intrigue check. In all cases, the DC is your Control DC + 2.`,
  ...
  "repair-the-flooded-mine": {
    ...
    description: `A group of engineers work to drain the flooded mine, shore up its damaged section, and establish the mine as a functional Work Site. ${loseRP(
      20
    )}, then attempt a DC 32 Engineering check.`,
    ...
    criticalFailure: {
      msg: `Work on repairing the mine proceeds, but at a much slower (and more expensive) pace than you’d hoped. The mine remains flooded, but you can attempt this check again on the next Kingdom turn. In addition, a catastrophic failure costs the lives of several workers. ${gainUnrest(
        2
      )} and ${gainRuin(
        "decay",
        1
      )}. The next time you attempt to Repair the Flooded Mine, it costs 40 RP.`,
    },
  },
  "request-foreign-aid": {
    oncePerRound: false,
    ...
    requirement:
      "You have diplomatic relations with the group you are requesting aid from.",
    skills: simpleRank(["statecraft"], 1),
    criticalSuccess: {
      msg: `Your ally’s aid grants a +4 circumstance bonus to any one Kingdom skill check attempted during the remainder of this Kingdom turn. You can choose to apply this bonus to any Kingdom skill check after the die is rolled, but must do so before the result is known. In addition, ${gainRolledRD(
        2
      )}; this RP does not accrue into XP at the end of the turn if you don’t spend it.`,
      modifiers: () => [
        {
          turns: 1,
          consumeId: "",
    ...
    success: {
      msg: `Your ally’s aid grants you either a +2 circumstance bonus to any one Kingdom skill check attempted during the remainder of this Kingdom turn or ${gainRolledRD(
        1
      )}. This RP does not accrue into XP at the end of the turn if you don’t spend it. You can choose to apply the bonus to any Kingdom skill check after the die is rolled, but must do so before the result is known.`,
      modifiers: () => [
        {
          turns: 1,
          consumeId: "",
...
You take time to relax, and you extend the chance to unwind to your citizens as well. The Kingdom skill you use to determine the effectiveness of your time off depends on how you want to spend it: Use a basic Arts check to spend the time engaged in entertainment or the pursuit of a hobby. Use a basic Boating check to enjoy trips on the lakes and rivers of your kingdom. Use a basic Scholarship check to spend the time reading or studying a topic of personal interest beyond your daily duties. Use a basic Trade check to spend your time shopping or feasting. Use a basic Wilderness check to get away from the bustle and relax in the countryside. If your kingdom Rested and Relaxed the previous Kingdom turn, the DC increases by 4, as your kingdom’s production and output hasn’t had a chance to catch up to all those vacation days.`,
    ...
    criticalSuccess: {
      msg:
        "The citizens enjoy the time off and are ready to get back to work. " +
        loseUnrest(1) +
        ", and the next Leadership activity you take gains a +2 circumstance bonus.",
      modifiers: () => [
        {
          turns: 1,
          consumeId: "",
  ...
  "restore-the-temple": {
    ...
    description: `You work with several worshippers of the temple's faith, gifted masons, and skilled laborers to restore the temple and once more consecrate it as a sacred place devoted to the worship of their god. ${loseRP(
      "1d6"
    )}, then attempt a DC 25 Folklore check.`,
    skills: simpleRank(["folklore"]),
    criticalSuccess: {
    ...
    criticalFailure: {
      msg: `Disaster strikes as the temple’s cavern collapses and rubble spills out to bury and destroy much of the temple’s plaza. ${gainRuin(
        "decay",
        1
      )} and ${gainUnrest(
        "1d4"
      )}. You can still attempt to Restore the Temple, but the DC for success increases by 4. This increase is cumulative with successive critical failures.`,
    },
  },
  "send-diplomatic-envoy": {
    ...
    criticalFailure: {
      msg: `Disaster! Your envoy fails to reach their destination, is turned back at the border, or is taken prisoner or executed, at the GM’s discretion. The repercussions on your kingdom’s morale and reputation are significant. Choose one of the following results: ${gainUnrest(
        "1d4"
      )}, add 1 to a Ruin of your choice, or ${loseRolledRD(
        2
      )}. In any event, you cannot attempt to Send a Diplomatic Envoy to this same target for the next 3 Kingdom Turns.`,
    },
  },
  "show-of-force": {
    oncePerRound: false,
    ...
    title: "Show of Force",
    description:
      "Using your advice, a public show of force is performed in an attempt to curtail criminal activity or subversive activity in the kingdom. Attempt a basic Warfare check to determine how effective the Show of Force is.",
    skills: simpleRank(["warfare"]),
    criticalSuccess: {
      msg: `The kingdom’s criminals are cowed by the show of force. ${loseRuin(
        "crime",
        2
      )} and reduce one other Ruin of your choice by 1.`,
    },
    success: {
      msg: `The kingdom’s criminals take note of the show of force. ${loseRuin(
        "crime",
        1
      )}`,
    },
    failure: {
      msg: `The show of force fails to impress criminals but unsettles the rest of the citizens. ${gainUnrest(
        1
      )}.`,
    },
    criticalFailure: {
      msg: `The criminals are emboldened by the failed show of force. ${gainUnrest(
        "1d4"
      )} and ${gainRuin("crime", 1)}.`,
    },
  },
  "spread-the-legend": {
    oncePerRound: false,
    ...
    description:
      "You work to spread the word of the party’s heroics and achievements, both through word of mouth and by distributing chapbooks or one-sheets detailing their exploits. Attempt a basic Arts check to determine the success of your efforts. If you have secured a printing press for the kingdom, the Arts check gains a +2 item bonus.",
    skills: simpleRank(["arts"]),
    criticalSuccess: {
      msg: `Not only do your stories bring pride and patriotism to the nation, but they also help increase its glory. ${loseUnrest(
        "1d6"
      )}, and ${createResourceButton({
        type: "fame",
        turn: "next",
        value: "1",
    ...
    success: {
      msg: `The rousing and inspiring stories you spread about the PCs helps to bring the nation together. ${loseUnrest(
        "1d6"
      )}`,
    },
    failure: {
      msg: `You avoid spreading unfortunate news, but only just barely. The citizens are only slightly entertained by their leaders’ exploits. ${loseUnrest(
        1
      )}.`,
    },
    criticalFailure: {
      msg: `You accidentally spreads news of a humiliating or embarrassing nature, causing the people of the kingdom to lose respect for their leaders. ${gainUnrest(
        "1d4"
      )}.`,
    },
  },
  "supernatural-solution": {
    ...
    skills: simpleRank(["magic"]),
    criticalSuccess: {
      msg: `You can call upon your spellcasters’ supernatural solution to aid in resolving any Kingdom skill check made during the remainder of this Kingdom turn ${gainSolution(
        "supernatural-solution"
      )}. Do so just before a Kingdom skill check is rolled (by yourself or any other PC). Attempt a Magic check against the same DC in addition to the Kingdom skill check, and take whichever of the two results you prefer. If you don’t use your Supernatural Solution by the end of this Kingdom turn, this benefit ends and you gain 10XP instead.`,
    },
    success: {
      msg: `You can call upon your spellcasters’ supernatural solution to aid in resolving any Kingdom skill check made during the remainder of this Kingdom turn ${gainSolution(
        "supernatural-solution"
      )}. Do so just before a Kingdom skill check is rolled (by yourself or any other PC). Attempt a Magic check against the same DC in addition to the Kingdom skill check, and take whichever of the two results you prefer. If you don’t use your Supernatural Solution by the end of this Kingdom turn, this benefit ends and you gain 10XP instead. However, you ${loseRP(
        "1d4"
      )} to research the solution. This cost is paid now, whether or not you use your supernatural solution.`,
    },
    failure: {
      msg: `Your attempt at researching a supernatural solution costs additional RP to research, but is ultimately a failure, providing no advantage. ${loseRP(
        "2d6"
      )}`,
    },
    criticalFailure: {
      msg: `Your attempt at researching a supernatural solution costs additional RP to research, but is ultimately a failure, providing no advantage. ${loseRP(
        "2d6"
      )}. In addition, your spellcasters’ resources and morale are impacted such that you cannot attempt a Supernatural Solution again for 2 Kingdom turns.`,
    },
    special:
      "You cannot influence a check with Supernatural Solution and Creative Solution simultaneously.",
  },
  "take-charge": {
    ...
    criticalSuccess: {
      msg: `${gainRP(
        1
      )}. In addition you get a +1 Circumstance Bonus to the next Check you make this turn with the chosen skill. `,
      modifiers: () => [
        {
          turns: 1,
          consumeId: "",
  ...
  "warfare-exercises": {
    ...
    description:
      "You spend time studying the nation’s armies, speaking with its commanders, researching historical records of battles, and running simulations in war rooms to help predict the best ways to prepare for upcoming conflicts. Attempt a basic Warfare check to determine the success of these exercises.",
    skills: simpleRank(["warfare"]),
    criticalSuccess: {
  ...
  "abandon-hex": {
    ...
    criticalSuccess: {
      msg: `You abandon the hex or hexes, decreasing your kingdom's Size by 1 per hex abandoned (this affects all statistics determined by Size; see page 532). Settlers and explorers return and resettle elsewhere in your kingdom, bringing with them bits of salvage from the abandoned hexes. ${gainRP(
        1
      )} per abandoned hex.`,
    },
    success: {
      msg: `You abandon the hex or hexes, decreasing your kingdom's Size by 1 per hex abandoned (this affects all statistics determined by Size; see page 532). Settlers and explorers return and resettle elsewhere in your kingdom, bringing with them bits of salvage from the abandoned hexes. ${gainUnrest(
        1
      )}`,
    },
    failure: {
      msg: `You abandon the hex or hexes, decreasing your kingdom's Size by 1 per hex abandoned (this affects all statistics determined by Size; see page 532). Some citizens become disgruntled refugees who refuse to leave the hex. ${gainUnrest(
        2
      )} and then attempt a @Check[type:flat|dc:6]. If you fail, the refugees become bandits, and during your next Event phase, your kingdom experiences a Squatters kingdom event automatically in addition to any other event that might occur.`,
    },
    criticalFailure: {
      msg: `You abandon the hex or hexes, decreasing your kingdom's Size by 1 per hex abandoned (this affects all statistics determined by Size; see page 532). Some citizens become disgruntled refugees who refuse to leave the hex. ${gainUnrest(
        3
      )} and automatically experience a Bandit Activity kingdom event.`,
    },
    special:
      "The Unrest gained from abandoning a hex doubles if it includes a settlement. A settlement in an abandoned hex becomes a Freehold (page 536).",
  },
  "build-roads": {
    title: "Build Roads",
    oncePerRound: false,
...
<p>Then attempt a basic check. Work with the GM to determine where your roads appear on the map.</p>`,
    requirement:
      "The hex in which you seek to build roads must be claimed by your kingdom.",
    ...
    criticalFailure: {
      msg:
        "Your attempt to build roads ends in disaster. Not only do you fail to build roads, but you lose several workers to an accident, banditry, a vicious monster, or some other unforeseen occurrence. " +
        gainUnrest(1),
    },
  },
  "claim-hex": {
    ...
    description: `Your surveyors fully explore the hex and attempt to add it into your kingdom’s domain. ${loseRP(
      1
    )} and then attempt a basic Exploration, Intrigue, Magic, or Wilderness check.`,
...
<p>If the hex you’re attempting to Clear has existing Ruins or an existing Structure, your action doesn’t physically remove the buildings from the area and you can later incorporate these buildings (or repair ruined ones) into a Settlement you build here later (see page 542). Regardless of the skill used, increase the basic DC by 2 if the hex to be cleared is not yet part of your kingdom.</p>`,
    ...
    criticalSuccess: {
      msg: `You successfully clear the hex. If you spent RP to attempt this activity, you’re refunded half of the RP cost. If you were removing dangerous creatures (but not hazards) from the hex, your explorers and mercenaries recover 2 Luxury Commodities as treasure. ${gainCommodities(
        "luxuries",
        2
      )}`,
    },
    ...
    criticalFailure: {
      msg: `You catastrophically fail to clear the hex and several workers lose their lives. ${gainUnrest(
        1
      )}`,
    },
  },
  "establish-farmland": {
    ...
    phase: "region",
    dc: "control",
    description: `You plant crops and establish livestock in permanent farms, ranches, and other growing operations to create Farmland (page 535). If you’re attempting to Establish Farmland in a hex that is predominantly plains, you must ${loseRP(
      1
    )} and the check is against your Control DC. If you’re targeting a hex that is predominantly hills, you must spend ${loseRP(
      2
    )} and the check is against your Control DC + 5.`,
    requirement:
  ...
  "establish-settlement": {
    ...
    criticalSuccess: {
      msg: `You establish the settlement largely with the aid of enthusiastic volunteers. ${loseRP(
        "1d6"
      )}`,
    },
    success: {
    ...
    failure: {
      msg: `You establish the settlement, but inefficiently and at great expense. ${loseRP(
        "6d6"
      )}`,
    },
    criticalFailure: {
...
<p>Then attempt a basic check. Lumber camps can be established in any hex that contains a significant amount of forest terrain. Mines and quarries can be established in any hex that contains a significant amount of hill or mountain terrain.</p>`,
    ...
    criticalFailure: {
      msg: `Not only do you fail to establish a Work Site, but you lose several workers to an accident, banditry, a vicious monster, or some other unforeseen occurrence. ${gainUnrest(
        1
      )}`,
    },
  },
...
<p>Then attempt a basic check. A fortified hex grants an additional bonus in warfare (see Appendix 3), but also gives traveling PCs a place to rest that prevents wandering monsters from interrupting their rest.</p>`,
    requirement:
      "The target hex must be claimed by your kingdom and must not have a settlement in it.",
    skills: simpleRank(["defense"]),
    criticalSuccess: {
      msg: `You find a defensible position for your fortification and finish construction efficiently. Gain a refund of half the RP you spent to build in the hex, then ${loseUnrest(
        1
      )}`,
    },
    success: {
      msg: `You establish your fortification in the hex. ${loseUnrest(1)}`,
    },
    failure: {
      msg: "You fail to fortify the hex.",
    },
    criticalFailure: {
      msg: `Your attempt ends in disaster. Not only do you fail to build a structure, but you lose several workers to an accident, banditry, a vicious monster, or some other unforeseen occurrence. ${gainUnrest(
        1
      )}`,
    },
  },
  "gather-livestock": {
    ...
    criticalFailure: {
      msg: `${loseCommodities(
        "food",
        "1d4"
      )} to spoilage. If you have no Food to lose, you instead ${gainUnrest(
        1
      )}`,
    },
  },
  "go-fishing": {
    ...
    requirement:
      "Must have at least one claimed hex that includes river or lake terrain.",
  ...
  "harvest-lily-pollen": {
    ...
    success: {
      msg: `You create 1 dose of @UUID[Compendium.pf2e.equipment-srd.Item.MvMa010Je10GN5dx]{Azure Lily Pollen} but must also attempt a @Check[type:basic|dc:11|showDC:all]. On a failure, some of the poison makes its way into criminal hands; ${gainRuin(
        "crime",
        1
      )}`,
    },
    failure: {
      msg: `You fail to harvest any @UUID[Compendium.pf2e.equipment-srd.Item.MvMa010Je10GN5dx]{Azure Lily Pollen}, in part because many of the resources make their way into the kingdom’s criminal underworld. ${gainRuin(
        "crime",
        1
      )}`,
    },
    criticalFailure: {
      msg: `Not only do you fail to harvest any pollen, and not only do resources make their way into the hands of criminals, but whispers and rumors that you allowed this to happen on purpose spread through the kingdom. ${gainUnrest(
        "1d4"
      )}, ${gainRuin("corruption", 1)}, and ${gainRuin("crime", 2)}`,
    },
  },
  "harvest-crops": {
    title: "Harvest Crops",
...
<p>Then attempt a basic check.</p>`,
    ...
    criticalFailure: {
      msg: `You fail to build workable systems or to restore a previous critical failure, and the hex does not gain the river or lake terrain feature. Your attempts at Irrigation are so completely useless that they become breeding grounds for disease. ${gainUnrest(
        1
...
Your kingdom can support 1 Recruited Monster at a time. If your kingdom is master in Intrigue, you can support up to 2 Recruited Monsters at a time, and if your kingdom is legendary in Intrigue, you can support up to 3 Recruited Monsters at a time.`,
  ...
  "supplementary-hunting": {
    ...
    description:
      "Following your advice, rural-dwelling citizens work to supplement stores of food and resources through hunting and trapping. Attempt a basic Wilderness check to gather excess livestock from the local wildlife, ranches, and farms to generate food commodities.",
    skills: simpleRank(["wilderness"]),
    criticalSuccess: {
      msg: `${gainCommodities("food", "1d4")}, ${gainCommodities(
        "luxuries",
        1
      )}, and ${createResourceButton({
        type: "resource-dice",
    ...
    success: {
      msg: `Choose one: ${gainCommodities("food", "1d4")}, ${gainCommodities(
        "luxuries",
        1
      )}, or ${createResourceButton({
        type: "resource-dice",
        turn: "next",
        value: "1",
      })}`,
    ...
    criticalFailure: {
      msg: `Your hunters and trappers fail to supplement your stores and must spend time resupplying and setting new traps; you cannot attempt Supplementary Hunting on the next Kingdom turn. In addition, your hunters and trappers have accidentally attracted the attention of dangerous wildlife. Either ${gainUnrest(
        "1d4"
      )} or increase a Ruin of your choice by 1.`,
    },
  },
  //civic activities
  "build-structure": {
...
`,
  ...
  demolish: {
    ...
    criticalFailure: {
      msg: `You fail to demolish the lot. It remains in Rubble and cannot be used for further construction until you successfully Demolish it. In addition, accidents during the demolition cost you the lives of some of your workers. ${gainUnrest(
        1
      )}`,
    },
  },
...
You must be at least master in Magic to attempt a Magic check. When you do so, choose any location within 30 hexes of the army’s current hex, then roll your check. If the army’s deployment causes it to cross your kingdom’s border, the DC increases by 5. If the army’s deployment causes it to cross an enemy kingdom’s border, the DC instead increases by 10.`,
    ...
    criticalFailure: {
      msg: `Rather than arriving at its destination, the army becomes lost until it recovers from this condition. ${gainUnrest(
        "1d4"
      )}, and attempt a @Check[type:flat|dc:11]; on a failure, reduce the army’s HP by 1.`,
    },
  },
  "disband-army": {
    oncePerRound: false,
  ...
  "garrison-army": {
    ...
    requirement:
      "The army is in the same hex as a Refuge, Settlement, or Work Site.",
    ...
    criticalFailure: {
      msg: `Your army clashes with local citizens, abuses their authority, lets their watchful readiness slack, and/or provokes confrontations where they are not needed. It does not become fortified, and you cannot attempt to garrison that army at this location again for 4 Kingdom turns. ${gainUnrest(
        1
      )}`,
    },
  },
...
If you’re distributing gear gained from battle, this activity requires a basic Warfare check and does not cost RP.`,
  ...
  "recover-army-damaged": {
    ...
    criticalFailure: {
      msg: `You fail to remove the affliction and your soldier’s lowered morale spreads discontent; ${gainUnrest(
        1
      )}.`,
    },
  },
...
export function gainCommodities(
  type: keyof Commodities,
  value: number | string
): string {
  return createResourceButton({ value: `${value}`, type });
}

export function loseCommodities(
  type: keyof Commodities,
  value: number | string
): string {
...
export function gainSolution(
  type: "creative-solution" | "supernatural-solution"
): string {
...
export function createResourceButton({
  ...
  const label = `${mode === "gain" ? "Gain" : "Lose"} ${value} ${unslugify(
    type
  )}${turnLabel}`;
...
export function getKingdomActivitiesById(
  additionalActivities: KingdomActivity[]
): KingdomActivityById {
  return <KingdomActivityById>(
    Object.fromEntries(
      getKingdomActivities(additionalActivities).map((activity) => [
        activity.id,
        activity,
      ])
    )
...
export function getKingdomActivities(
  additionalActivities: KingdomActivity[]
): KingdomActivity[] {
